...
class RyuSwitch(object):
    ...
    def get_port_description(self, port=None, openflow=None):
        ...
        if port is not None and openflow is None:
            print(
                "[ WARNING ]: Port filter specified, but OpenFlow version was not. Filtering is only compatible with OpenFlow v1.5+. Defaulting to no filter & dumping all ports.")
        elif port is not None and openflow < 1.5:
            print(
                "[ WARNING ]: Port filter specified, but REST API does not allow filtering with specified OpenFlow version (v" + str(
                    openflow) + "). Filtering is only compatible with OpenFlow v1.5+. Defaulting to no filter & dumping all ports.")
        elif port is not None and openflow >= 1.5:
    ...
    # Uses Ryu's rest_topology API to get switches/links/hosts learned from topology discovery
    # Compare https://github.com/faucetsdn/ryu/blob/master/ryu/app/rest_topology.py
    # Run ryu-manager with rest_topology app and --observe-links

    def topo_get_switches(self):
        """
        Description:
        ...
        # Path: /stats/switches
        rest_uri = self.API + "/v1.0/topology/switches"
    ...
    def topo_get_links(self):
        ...
        # Path: /stats/switches
        rest_uri = self.API + "/v1.0/topology/links"
    ...
    def topo_get_switch(self, dpid):
        ...
        # Path: /stats/switches
        rest_uri = self.API + f"/v1.0/topology/switches/{dpid}"
    ...
    def topo_get_switch_links(self, dpid):
        ...
        # Path: /stats/switches
        rest_uri = self.API + f"/v1.0/topology/links/{dpid}"
    ...
    def topo_get_hosts(self):
        ...
        # Path: /stats/switches
        rest_uri = self.API + f"/v1.0/topology/hosts"
    ...
    def topo_get_switch_hosts(self, dpid):
        ...
        # Path: /stats/switches
        rest_uri = self.API + f"/v1.0/topology/hosts/{dpid}"
